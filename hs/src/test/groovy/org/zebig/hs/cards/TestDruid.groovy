...
import static org.zebig.hs.mechanics.buffs.BuffType.getTAUNT
...
class TestDruid extends TestHelper {
    ...
    @Test
    void MarkOfNature_choice1_attack_increase() {
    ...
    @Test
    void MarkOfNature_choice2_taunt() {
        // Choose One - Give a minion +4 Attack; or +4 Health and Taunt
    ...
    void SoulOfTheForest_play() {
        ...
        _next_turn()
        (0..2).each {
            assert !p1.board.get(it).is_dead()
        }
        _next_turn()
        _next_turn()
        (0..2).each {
    ...
    void WildGrowth_10_mana() {
        ...
        assert p1.hand.get(0).name == "Excess Mana"
        p1.play(p1.hand.get(0)) /// draw a card
        assert p1.hand.size() == 1
        assert p1.hand.get(0).name != "Excess Mana"
    }
